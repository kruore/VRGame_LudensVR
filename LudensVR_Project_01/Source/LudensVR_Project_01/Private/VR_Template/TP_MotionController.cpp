...
ATP_MotionController::ATP_MotionController()
{
	...
	static ConstructorHelpers::FObjectFinder<UClass> AnimBP_HandAnimation(TEXT("Class'/Game/LudensVR/Anim/Glove_VR_Hand_AnimBP.Glove_VR_Hand_AnimBP_C'"));
	//static ConstructorHelpers::FObjectFinder<UClass> AnimBP_HandAnimation(TEXT("Class'/Game/VirtualReality/Mannequin/Animations/AnimBP_RightHand.AnimBP_RightHand_C'"));
	...
	static ConstructorHelpers::FObjectFinder<USkeletalMesh> SK_Hand(TEXT("SkeletalMesh'/Game/IBFPModernCivilianHands/Hands_VR/SKM_VR_Male_Motorcycle_Gloves_R.SKM_VR_Male_Motorcycle_Gloves_R'"));
	...
	//-----
	RayCastShootTR = CreateDefaultSubobject<USceneComponent>(TEXT("RayCastShootTR"));
	RayCastShootTR->SetupAttachment(HandMesh);
	//RayCastShootTR->SetWorldTransform(HandMesh->GetComponentTransform());
	RayCastShootTR->SetRelativeLocation(FVector(25.0f, 0.0f, 0.0f));
	...
	ThumbPoint = CreateDefaultSubobject<USceneComponent>(TEXT("ThumbPoint"));
	ThumbPoint->SetupAttachment(GrabShpere);

	m_playerHand = CreateDefaultSubobject<ULV_PlayerHand>(TEXT("Player"));
...
void ATP_MotionController::GrabActor()
{
	...
	if (m_playerHand->GrabObject(&MotionController, this))
	{
		RumbleController(0.3f);
	}

	//GEngine->AddOnScreenDebugMessage(-1, 5.0, FColor::Red, FString::Printf(TEXT("%s"), *m_playerHand->currentWeaponHolder->Tags[0].ToString()));
...
void ATP_MotionController::ReleaseActor()
{
	...
	if (m_playerHand->ReleaseObject(&MotionController))
	{
...
void ATP_MotionController::PlayFireAnim()
{
	//HandMesh->PlayAnimation(pistolShotAnimSequence, false);
}